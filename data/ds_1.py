...
class KeypointAugmentor:
    ...
    def apply_rotation(self, keypoints):
        ...
        # Estimate center: mean of all valid keypoints
    ...
    def apply_flip(self, keypoints):
        ...
        # Assume x is horizontal; negate x around center
        valid = ~torch.isnan(keypoints[..., 0])
        if valid.any():
            center_x = keypoints[..., 0][valid].mean()
        else:
...
class CustomDataset(Dataset):
    ...
    def compute_feature(self, keypoints):
            ...
            FPS = 30  # Adjust based on your video framerate
            ...
            # Add inter-mouse features from the perspective of each mouse
            for i in range(M):
                for j in range(M):
