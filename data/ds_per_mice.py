...
class CustomDataset(Dataset):
        ...
        for idx in tqdm(range(len(self.df)), desc="Scanning videos and pre-loading labels"):
            ...
            if os.path.exists(features_path) and os.path.exists(labels_path) and num_frames > 0:
                ...
                self.video_info[video_id] = {
                    ...
                    'lab_id': row['lab_id'],
                    'behavior_frames': behavior_frames,  # Store the list of positive frames
    ...
    def __getitem__(self, idx):
        ...
        # Load data with mmap for efficiency
        ...
        mice_mask = ~torch.isnan(torch.sum(features, dim=(2,3)))


        if labels_mmap is not None and self.mode == 'train':  # Only load labels for train
        ...
        # Compute features
        features, feature_mask = self.compute_feature(features)
    ...
    def compute_feature(self, keypoints):
        ...
        # Stack across mice dimension
        features = torch.stack(per_mouse_feats, dim=1)  # [T, M, 58]
...
def collate_fn(batch):
    ...
    mice_masks = torch.stack([item['mice_mask'] for item in batch]) if 'mice_mask' in batch[0] else None
    feature_masks = torch.stack([item['feature_mask'] for item in batch]) if 'feature_mask' in batch[0] else None
    feature_masks = torch.flatten(feature_masks, start_dim=-2, end_dim=-1) 
